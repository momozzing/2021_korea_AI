'''
deepspeed --num_gpus=1 BoolQ_post_train_fine_tune.py
'''
...
ckpt_name = "model_save/tunib-electra-ko-base-BoolQ-0-1234-post_harf.pt"                                    ###
...
wandb.init(project="GPT-finetune", name=f"BoolQ-{model_name}-{random_seed}-post_harf_1_v14")
...
for epoch in range(args.epoch):
    ...
    for idx, eval in tqdm(enumerate(eval_loader)):
        ...
        # print(lossss)
            ...
            torch.save(model.state_dict(), f"model_save/{model_name.replace('/', '-')}-{task}-{step}-{epochs}-{random_seed}-post_harf_1_v14.pt")
