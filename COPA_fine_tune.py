'''
deepspeed --num_gpus=1 COPA_fine_tune.py
'''
...
parser.add_argument("--deepspeed_config", type=str, default="COPA_ds_config.json")
...
wandb.init(project="GPT-finetune", name=f"COPA-{model_name}-50-256-{random_seed}-v11")
train_data = pd.read_csv("data/COPA_data/COPA_train_pt_dev_pt_translation.tsv", delimiter="\t")
...
eval_data = pd.read_csv("data/COPA_data/SKT_COPA_Test_labeling.tsv", delimiter="\t")
...
for epoch in range(args.epoch):
    ...
    torch.save(model.state_dict(), f"model_save/{model_name.replace('/', '-')}-{task}-{epoch}-50-256-{random_seed}-v11.pt")
