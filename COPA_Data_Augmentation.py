...
train_data = pd.read_csv("data/COPA_data/SKT_COPA_Train.tsv", delimiter="\t")
...
for idx in tqdm(range(len(train_data))):
    text, question, fir, sec, labels = train_data["sentence"][idx], train_data["question"][idx], train_data["1"][idx], train_data["2"][idx], train_data["Answer"][idx] 

    text_result = aeda(text)
    fir_result = aeda(fir)
    sec_result = aeda(sec)

    all_list.append([text_result] + [question] + [fir_result] + [sec_result] + [labels])
    
 

df = pd.DataFrame(all_list, columns=['sentence', 'question', '1', '2', 'Answer'])

# print(df)

aug_df = pd.concat([train_data, df])

# aug_df.to_csv('data/COPA_data/COPA_Aug_train.tsv', index=False, sep="\t")

# print(len(aug_df['Answer']))
###################################### EDA
